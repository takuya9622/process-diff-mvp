...
最初のネイティブ業務実行は、既存の経費精算サンプルを使って次を成立させました。
対応する次期MVP画面は
...
- 固定経路は申請者、承認者、経理担当とし、初回サンプルでは同じ認証済みmemberが兼務する。
  実利用者と業務役割を別々に記録し、複数memberの割当へ置き換えられるようにする。
- 添付ファイル本体は含めず、領収書の参照情報を必須項目として検証する。
- 期限はWorkItem作成時刻からの暦日で計算する。
- Activityを利用者向け履歴として保存し、完全な監査イベントは別概念のまま後続とする。

### 14.3 後続で決める事項

- `WorkflowDefinition`と`WorkflowVersion`の編集・公開権限を現行roleへどう割り当てるか
- W-13で業務役割、上長判定、代理、職務分離をどう設定するか
- BusinessCalendar、並列承認、待機、外部同期状態をどの順で追加するか
