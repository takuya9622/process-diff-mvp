...
| 項目 | 内容 |
|---|---|
| 文書状態 | 業務理解・安全な変更・経費申請完了シナリオ実装済み |
| 対象 | MVPで使用するサンプル業務、seed、デモとE2Eの基準シナリオ |
| 最終更新日 | 2026-08-10 |

この文書では、MVPの初期状態として使用するサンプル業務と、変更しない業務理解、安全な変更、
経費申請の完了を確認する三つの基準シナリオを定義します。影響候補の算出規則は
...
組織ワークスペースの初期表示では`process-expense-submission`（経費申請）を選択します。
利用者は変更操作をせず、登録済みの関係から次を確認します。

- 従うルール: 領収書提出ルール、金額別承認ルール
- 使用システム: 経費申請システム
- 担当・承認: 申請者、承認者
- 関連文書: 経費申請マニュアル
- 後工程: 証憑確認

種類別一覧を眺めるだけでなく、経費申請をハブに周辺要素を理解し、任意の関連要素から
経費申請へ戻れることを基準とします。このシナリオでは編集、差分、変更履歴を表示しません。

### 5.2 安全な変更シナリオ

業務理解シナリオから`rule-receipt-requirement`（領収書提出ルール）を開き、この要素を
変更対象とします。
...
1. 初期表示で「経費申請」の現在内容と、操作中の組織、利用者、権限を確認できる。
2. 変更操作をせず、経費申請に関係するルール2件、システム1件、役割2件、文書1件、
   後工程1件を意味ごとのまとまりで確認できる。
3. 任意の関連要素を開き、その要素に関係する業務として経費申請へ戻れる。
4. 通常利用だけを見ても、文書の保管ではなく業務構造をたどる体験だと説明できる。

### 7.2 安全な変更

1. 経費申請から領収書提出ルールを開き、直接関係する要素を確認できる。
...
固定サンプルでは同じ認証済みmemberが申請者、承認者、経理担当を兼務し、役割ごとの画面と
Activityを検証します。

1. ホームまたは業務カタログから公開済みの「経費申請」を選ぶ。
2. 経費発生日、金額、用途、支払先、領収書参照情報を入力して申請する。
3. 案件詳細で、承認待ちの現在地、承認者、期限、適用workflow versionを確認する。
4. 承認作業で理由を入力して差し戻す。
5. 同じ案件の修正作業で差し戻し理由を確認し、用途を修正して再申請する。
6. 新しい承認作業で申請内容と判断の根拠を確認し、理由付きで承認する。
7. 経理作業で処理日、処理参照番号、処理結果・証跡を記録して完了する。
8. 完了案件を再表示し、申請値、2回の承認履歴、経理結果、全Activityを確認する。
9. 処理済みの承認作業を再実行しても状態が変わらず、別組織から案件IDを指定しても取得できない。

### 7.4 ネイティブコミュニケーション

組織作成時に「お知らせ・運用」と「経費・申請相談」のチャンネルを作成します。

1. サイドバーからコミュニケーションを開き、組織内チャンネルを選択する。
2. 「支払い処理相談」チャンネルを作成する。
3. 進行中の経費申請を添えて、支払い予定の確認メッセージを投稿する。
4. 投稿を再表示し、案件番号、業務名、状態を確認する。
5. 共有案件を開き、同じ案件詳細とActivityへ戻る。
6. SlackまたはGoogle Chatが未接続でも1〜5を完了できる。

四つのシナリオを、domain logicのunit test、integration test、Playwrightの中核E2E、第三者へ説明するデモ手順の
共通基準とします。
