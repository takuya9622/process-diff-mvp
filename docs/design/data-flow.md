...
算出規則は[影響候補の探索仕様](impact-search.md)、共有状態とリセットは
...
初期MVPのD1とD2は、一つの共有サンプル状態として扱います。
...
変更確定後は、確定した差分、影響候補、候補になった理由と関係経路を、SPA内の一つの
変更結果として表示します。確定済みの`ChangeSet`を表示の識別子とし、ブラウザ更新後も
同じ結果を再取得できる構造にします。具体的な画面状態は
[MVP画面構成とユーザーフロー](screens-and-user-flow.md)で定義します。

### 7.6 公開デモは一つの共有状態を使う

初期MVPでは、すべての利用者がD1とD2の同じ状態を参照・変更します。同じ業務要素への
古いversionを基準とした更新は競合として拒否し、入力中の変更案はブラウザに保持します。

### 7.7 リセットは初期状態を再作成する

リセットでは、現在の業務グラフ、version、変更履歴を一つのトランザクションで削除し、
D0から初期versionを再作成します。リセット自体を変更履歴へ記録しません。競合とリセットの
詳細は[公開デモの状態管理](demo-state.md)で定義します。
